...
def render():
    ...
    options = {
        f"{a['account_type'].capitalize()} #{a['account_id']}": a["account_id"]
        for a in accounts
    }
    ...
    default_index = (
        list(options.keys()).index(preselect_label) if preselect_label else 0
    )
    selected_label = st.selectbox(
        "Select account", list(options.keys()), index=default_index
    )
    selected_id = options[selected_label]
    st.session_state.selected_account_id = selected_id
    ...
    if not transactions:
        ...
        for txn in transactions:
            ...
            color = "#A32D2D" if is_outgoing else "#0F6E56"
            label = (
                f"Transfer to Acc #{txn['to_account_id']}"
                if is_outgoing
                else f"Received from Acc #{txn['from_account_id']}"
            )
